...
<p>Länk till projektsidan: <a href="rm-home.php">film.du</a>
</p>
...
<p>Detta krav implementerade jag i två steg. I det första steget utgick jag från mitt Loom-ramverk från kmom06 och gjorde jag en väldigt enkel logga med huvudtitel och slogan samt en enkel footer. För att kunna göra resten behövde jag först göra färdigt Film-sidan och cms-delen för sidor och blog-posts som även ingick i krav 2 3, och 4. Efter att dessa var klara var det enkelt uppdatera menyn för att visa rätt sidor.
</p>
...
<p>Style har jag valt att göra enkel och ljus med olika ljusgrå nyanser. De olika delarna av webbsidan (header, film-kort, blogg-inlägg, footer) ligger i regel som ett slags kort ovanpå bakgrunden. Sidorna är inte uttestade för mobilt bruk utan vill helst ha en bredd större än 850 pixlar. Version 2 får bli mer responsiv även rekommendationen är att designa för liten skärm först.
</p>
<p>Style på admin-sidorna är stylade speciellt mycket.
</p>
...
<!-- Krav 3: Sida - Nyheter -->
<h3></h3>
<p>Här utgick jag också från blogg-delen från övning kmom05. Den stora utökningen var att lägga till kategori på blogg-inläggen. Då det bara behövdes en kategori per inlägg var det relativt lätt att lägga till. För filmgenrer blir det mer komplext med en-till-många mappning mellan film-genrer.
</p>
<!-- Krav 4: Första sidan -->
<h3>Krav 4: Första sidan</h3>
<p>Första sidan skapade jag med en ny page controller. En tanke var att använda page.php och lagra sid-innehåll i databasen Detta hade då krävt stöd av någon slags kortkommandon som identifieras i textfilter-funktionen och då kan köra någon utpekad php-funktion som genererar html. Istället fick det bli php rätt upp och ner i controllern.
</p>
<p>För vart och ett av kraven i första sidan har det krävts mindre utökningar och ändringar av t.ex. CBlog och film-db-klasserna. T.ex. i CContent::GetPosts lade jag till en parameter för att begränsa antalet returnerade rader. Detta görs genom att lägga till "... LIMIT 3" i sql-satsen.
</p>
<!-- Krav 5, 6: Extra funktioner (optionell) -->
<h3>Extra funktioner</h3>
<h4>Tävling</h4>
<p>Tärningsspelet hade jag gjort i en tidigare uppgift så detta var ganska enkelt att lägga till. Text till spelet behövde dock ändras något. Även utkomst av spelet där en film presenteras som en vinst behövde läggas till. Istället för att ändra befintlig klass C100Game, så prövade jag att ärva C100Game till en ny klass CRM100Game. Jag överlagrade metoden gameHTML lite annorlunda text samt presentation av film vid vinst. Gemensam instruktions-text refaktorerade jag ut till en konstant egenskap så den kunde ärvas in i CRM100Game. För att kunna hämta en film behövde klassen tillgång till film-databasen. För detta lade jag till en metod för att sätta databas som en egenskap i klassen. Sist fick jag ändra style för spelet för att passa in i film-sitens utseende.
</p>
<p>
</p>
<h4>Kalender</h4>
<p>För att göra kalendern utgick jag ifrån DateTime-objektet. Med detta tog jag fram olika dagar för att kunna rita upp kalendern, t.ex. dagens datum, första/sista dagen i månaden, veckan för första dagen i månaden och den första dagen (måndag) i denna vecka, osv. DateTime har en format-metod för att skapa text-strängar med datum/tid t.ex. "Mon 3 June". DateTime verkar dock bara kunna skapa engelsk text. Jag använde därför strftime funktionen som tillsammans med setlocale(LC_TIME, "Swedish") gör att man får ut datum/tid med svensk locale. Kodningen ut från strftime var fel så jag fick koda om till utf-8 med utf8_encode, alltså <code>utf8_encode(strftime("%A %#d %B", strtotime(\$date->format('Y-m-d'))))</code> för att komma till rätt text-sträng t.ex. "fredag 8 januari". Detta fungerade fint på min lokala Windows-maskin, men jag lyckades inte få BTH's server att få ut svenska, bara engelska. Jag hann inte felsöka närmre i det.
</p>
<p>För månades film återanvände jag samma "film-korts"-generering som jag använder på den vanliga film-sidan, förutom viss annorlunda style just på kalender-sidan.
</p>
<h4>Ladda upp bilder</h4>
<p>Uppladdningsfunktionalitet lade jag i en ny metod "UploadImageFile" i klassen CRMMovieAdmin som hanterar skapande/uppdatering av övrig data för en film. Jag utgick från ett exempel på W3schools. Det är samma page-controller som anropas från formuläret vid uppladdning som vid skapande/uppdatering av film. Page-controllern testar om användaren har tryckt på upload knappen och laddar då upp filen. Efter uppladdningen till servern sparas också övriga fält till databasen som användaren fyllt i tillsammans med filnamnet till bilden. Ett problem jag haft med BTH servern är access-rättighet till uppladdnings-foldern. Jag har chmod'at den till 777 för att flyttning från temp-folder till uppladdn.-folder ska fungera. En något säkrare lösning bör vara att ge www-data användaren grupp-access till foldern vilket gör att det räcker med permission 664 för upload-foldern.
</p>
<p>Not: just nu finns en liten snagg i page-controllern som inte ser skillnad på om anropet kommer från create- eller ifrån update-formuläret när användaren trycker på upload-knappen. Bör uppdateras med något dolt fält från formuläret som talar om ifall det var update eller create formuläret som postades.
</p>
